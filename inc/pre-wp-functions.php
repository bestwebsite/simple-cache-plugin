 ...
 * @return string
 */
function bestwebsite_file_cache( $buffer, $flags ) {
	global $post;

	$cache_dir = bestwebsite_get_cache_dir();
	...
	if ( ! file_exists( $cache_dir ) ) {
		...
		$buffer = apply_filters( 'bestwebsite_pre_cache_buffer', $buffer );
	}

	$url_path = bestwebsite_get_url_path();
	...
	if ( ! empty( $GLOBALS['bestwebsite_config']['enable_gzip_compression'] ) && function_exists( 'gzencode' ) ) {
	...
	if ( function_exists( 'ob_gzhandler' ) && ! empty( $GLOBALS['bestwebsite_config']['enable_gzip_compression'] ) ) {
 ...
 * @return string
 */
function bestwebsite_get_url_path() {
 ...
 * @return string
 */
function bestwebsite_get_cache_path() {
	return rtrim( WP_CONTENT_DIR, '/' ) . '/cache/bestwebsite-simple-cache';
}
 ...
 * @since 1.0
 */
function bestwebsite_serve_file_cache() {
	$cache_dir = ( defined( 'bestwebsite_CACHE_DIR' ) ) ? rtrim( bestwebsite_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/bestwebsite-simple-cache';

	$file_name = 'index.html';

	if ( function_exists( 'gzencode' ) && ! empty( $GLOBALS['bestwebsite_config']['enable_gzip_compression'] ) ) {
		$file_name = 'index.gzip.html';
	}

	$path = $cache_dir . '/' . rtrim( bestwebsite_get_url_path(), '/' ) . '/' . $file_name;
 ...
 * @return string
 */
function bestwebsite_get_cache_dir() {
	return ( defined( 'bestwebsite_CACHE_DIR' ) ) ? rtrim( bestwebsite_CACHE_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/cache/bestwebsite-simple-cache';
}
 ...
 * @return string
 */
function bestwebsite_get_config_dir() {
	return ( defined( 'bestwebsite_CONFIG_DIR' ) ) ? rtrim( bestwebsite_CONFIG_DIR, '/' ) : rtrim( WP_CONTENT_DIR, '/' ) . '/sc-config';
}
 ...
 * @return string
 */
function bestwebsite_get_config_file_name() {
 ...
 * @return bool|array
 */
function bestwebsite_load_config() {
	if ( @file_exists( bestwebsite_get_config_dir() . '/config-network.php' ) ) {
		return include bestwebsite_get_config_dir() . '/config-network.php';
	} elseif ( @file_exists( bestwebsite_get_config_dir() . '/' . bestwebsite_get_config_file_name() ) ) {
		return include bestwebsite_get_config_dir() . '/' . bestwebsite_get_config_file_name();
	}
 ...
 * @return boolean
 */
function bestwebsite_url_exception_match( $exception, $regex = false ) {
