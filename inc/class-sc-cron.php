...
class bestwebsite_Cron {
	...
	private function setup() {
		...
		add_action( 'bestwebsite_purge_cache', array( $this, 'purge_cache' ) );
	...
	public function filter_cron_schedules( $schedules ) {
		...
		$schedules['simple_cache'] = array(
			'interval' => apply_filters( 'bestwebsite_cache_purge_interval', $interval ),
			'display'  => esc_html__( 'Simple Cache Purge Interval', 'bestwebsite-simple-cache' ),
		);
		return $schedules;
	}
	...
	public function unschedule_events() {
		$timestamp = wp_next_scheduled( 'bestwebsite_purge_cache' );

		wp_unschedule_event( $timestamp, 'bestwebsite_purge_cache' );
	}
	...
	public function schedule_events() {
		...
		if ( ! $timestamp ) {
			wp_schedule_event( time(), 'simple_cache', 'bestwebsite_purge_cache' );
		}
	}
