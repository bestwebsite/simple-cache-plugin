 ...
 */
class bestwebsite_Advanced_Cache {
	...
	public function set_comment_cookie_exceptions( $comment ) {
		...
		if ( ! empty( $config['enable_page_caching'] ) && empty( $config['enable_in_memory_object_caching'] ) ) {
			...
			setcookie( 'bestwebsite_commented_posts[' . $post_id . ']', wp_parse_url( get_permalink( $post_id ), PHP_URL_PATH ), ( time() + HOUR_IN_SECONDS * 24 * 30 ) );
		}
	}
	...
	public function purge_post_on_comment_status_change( $comment_id ) {
		...
		if ( ! empty( $config['enable_page_caching'] ) && empty( $config['enable_in_memory_object_caching'] ) ) {
			...
			$path = bestwebsite_get_cache_path() . '/' . preg_replace( '#https?://#i', '', get_permalink( $post_id ) );
	...
	public function clean_up() {
		...
		$folder = untrailingslashit( WP_CONTENT_DIR ) . '/cache/bestwebsite-simple-cache';
	...
	public function get_file_code() {
		...
		return '<?php ' .
		"\r\n" . "defined( 'ABSPATH' ) || exit;" .
		"\r\n" . "define( 'bestwebsite_ADVANCED_CACHE', true );" .
		"\r\n" . 'if ( is_admin() ) { return; }' .
		"\r\n" . "\$plugin_path = defined( 'WP_PLUGIN_DIR' ) ? WP_PLUGIN_DIR : WP_CONTENT_DIR . '/plugins/';" .
		"\r\n" . "include_once( \$plugin_path . '/bestwebsite-simple-cache/inc/pre-wp-functions.php' );" .
		"\r\n" . "\$GLOBALS['bestwebsite_config'] = bestwebsite_load_config();" .
		"\r\n" . "if ( empty( \$GLOBALS['bestwebsite_config'] ) || empty( \$GLOBALS['bestwebsite_config']['enable_page_caching'] ) ) { return; }" .
		"\r\n" . "if ( @file_exists( \$plugin_path . '/bestwebsite-simple-cache/inc/dropins/" . $cache_file . "' ) ) { include_once( \$plugin_path . '/bestwebsite-simple-cache/inc/dropins/" . $cache_file . "' ); }" . "\r\n";
		// phpcs:enable
	}
